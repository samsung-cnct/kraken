...
podTemplate(
    ...
    containers: [
        ...
        containerTemplate(name: 'install-make', image: 'ubuntu:16.04', ttyEnabled: true, command: 'cat', alwaysPullImage: true),
    ...
    volumes: [
      ...
      hostPathVolume(hostPath: '/opt/bin', mountPath: '/out'),
    ...
    ])
    {
        node('kraken') {
            ...
            customContainer('install-make') {
                kubesh "apt-get update && apt-get -y install make && cp /usr/bin/make /out/make"
            }

            withEnv(["GOPATH=${WORKSPACE}/go/"]) {
                stage('Make: build, install, compile') {
                    //sh "docker run -ti --rm -v /opt/bin:/out ubuntu:16.04 sh -c 'apt-get update && apt-get -y install make && cp /usr/bin/make /out/make'"
                    sh "which make"
                    sh "cd go/src/github.com/samsung-cnct/kraken/ && /opt/bin/make build KLIB_VER=${k2_image_tag}"
                    sh "cd go/src/github.com/samsung-cnct/kraken/ && /opt/bin/make install KLIB_VER=${k2_image_tag}"
                    sh "cd go/src/github.com/samsung-cnct/kraken/ && /opt/bin/make compile KLIB_VER=${k2_image_tag}"
                }
            }
        }
